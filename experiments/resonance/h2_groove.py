...
barrel/end-cap corner sees the slot mouth with input impedance
    ...
    Z_in = j Z0 tan(beta d),    beta = 2 pi / wavelength,  wavelength = 122.36 mm

so d = wavelength/4 = 30.59 mm is an OPEN circuit and blocks the current, and
d = wavelength/2 = 61.18 mm is a SHORT and the groove is invisible. Position of the
...
detuning, not the maximum the groove can deliver. wavelength/4 is over-design; the
...
VERIFICATION, declared before the run:
  ...
  2. the TM111 shift must follow tan(2 pi d / wavelength) — rising toward 30.59 mm.

FALSIFICATION:
  🔴 TE011 shifting comparably to TM111 -> the mechanism is wrong, whatever the
     splitting does.
  🔴 no inversion or roll-off past wavelength/4 -> it is not behaving as a stub, and
...
def main():
    ...
    print(f"  wavelength/4 = {WAVELENGTH/4:.2f} mm, wavelength/2 = {WAVELENGTH/2:.2f} mm\n", flush=True)
    ...
    for gd in DEPTHS:
        ...
        print(f"    depth {gd:6.2f} mm  ({gd/WAVELENGTH:.3f} wavelength)  "
    ...
    print("\n" + "=" * 78)
    print(f"  {'depth':>8}{'d/wavelength':>14}{'TE011 shift':>13}{'TM111 shift':>13}"
    ...
    for r in out:
        ...
        print(f"  {r['depth_mm']:>8.2f}{r['depth_mm']/WAVELENGTH:>14.3f}{dte:>13.2f}"
    ...
    if clears:
        ...
        print(f"  MINIMUM depth clearing the {BAND_MHZ:.0f} MHz band: "
              f"{mn['depth_mm']:.2f} mm ({mn['depth_mm']/WAVELENGTH:.3f} wavelength), "
              f"Q cost {(mn['q_te011']/base['q_te011']-1):+.1%}")
    else:
        print(f"  🔴 no depth tested clears {BAND_MHZ:.0f} MHz")
    json.dump({"dl": DL, "a": a, "L": L, "width_mm": WIDTH, "wavelength_mm": WAVELENGTH,
