...
TWO HARD ANCHORS. Everything else is dimensionless or reduces to it.
  ...
  ① f0 = 2.45 GHz, band 2.400-2.500 = ±2.04% FRACTIONAL.
     ...
     one. Fixes wavelength = 122.36 mm; every length becomes a ratio to it.
  ...
  ② N2 at 0-2 atm.
     ...
     sigma, which re-enters the electromagnetics ONLY as delta/t or delta/wavelength.

Two that LOOK like hard units and are not:
  wall conductivity -> delta_wall/wavelength = 1.05e-5, a fixed dimensionless number
...
wavelength would become the next thing to drift out of step — which is the failure
...
def view(path="baselines.json"):
    """Every length as a fraction of wavelength, every frequency as a fraction of f0."""
    ...
    for k, v in d.items():
        ...
        if isinstance(val, (int, float)) and not isinstance(val, bool):
            if u.startswith("mm"):
                out[k] = dict(kind="length", mm=val, over_wavelength=val / WAVELENGTH)
            elif u == "GHz":
            ...
            out[k] = dict(kind="length_list", mm=val,
                          over_wavelength=[x / WAVELENGTH for x in val])
    return out
...
if __name__ == "__main__":
    ...
    print(f"  {'sigma':>8}{'delta mm':>11}{'delta/t':>9}{'delta/wavelength':>18}")
    for s in (0.3, 1, 3, 10, 30, 100, 300):
        dl = skin_mm(s)
        print(f"  {s:>8g}{dl:>11.2f}{dl / 4.0:>9.2f}{dl / WAVELENGTH:>18.4f}")
