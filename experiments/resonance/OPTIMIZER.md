...
**2026-08-24.** Anchored to a measured N₂ MICAP gas temperature of **5220–5270 K**
(Kuonen et al., *JAAS* 39(5) 2024, Table 2, **pressure-reduction — the only
empirical method of the three**). The old `1e20` was solver-convenience and
**13× too high**.

🔑 **FOR THE SURROGATE:** n_e stops being a search axis and becomes a **narrow
input band**. `physics.plasma_state(T_gas)` derives n_e, ν_m and n_heavy from ONE
temperature, so the three can no longer disagree (§7ad).
🔴 **AND THE OPTIMUM DOES NOT SIT WHERE THE ASSUMED VALUE PUT IT.** Q₀ minimises
near 1e19; the assumed 1e20 was on the far side, the truth is just below.
**Band margin 17.6 → 41.4 MHz (better); VSWR 58 → 75–82 (worse, MEASURED
2026-08-25 — the interpolated 80–89 was high).** Any prior fitted
"at the operating point" describes the wrong point.
⚠️ **The residual uncertainty is now LTE, not the density.** Non-LTE puts n_e
ABOVE Saha — **asymmetric**, and it pushes toward the VSWR peak at 1e19. Score
that risk one-sided.

### The old framing, kept because the REASONING was right
