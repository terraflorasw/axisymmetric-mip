# INCIDENTS.md — the evidence behind CONVENTIONS

> Split out of `CONVENTIONS.md` on 2026-09-04. **Nothing here is canon.**
> Every entry is a mistake actually made, kept verbatim as the evidence for a
> rule. The RULES live in `CONVENTIONS.md`; if the two ever disagree,
> **CONVENTIONS wins and the entry below is stale.**

🔑 **Why this file exists.** `CONVENTIONS.md` reached **3,454 lines, 90 % of
it these narratives.** At that size it stopped being consulted, so its rules
were re-discovered and re-appended — which made it longer. §7bi (2026-08-25,
*"no constants in scripts"*) already covered the three constant-provenance
defects written up again as §7bx/§7by/§7bz on 2026-09-03/04. **The log was
feeding the failure it documented.**
...
| `cavity.Q_ext.cold` | 9,231 (no-torch eigen) · 9,117 (vacuum-torch eigen) · 8,462 (driven dip) |
...
**A name alone no longer resolves.** `get("cavity.Q_ext.cold")` raises and prints all
three with the discriminating keys; `get("cavity.Q_ext.cold", solver="driven_dip",
...
| | failure | example |
|---|---|---|
| 1 | **the sweep does not span the governing variable** | `h3-quarterwave-01`: `gap` enters L only ONCE, so with `ld` fixed all three cases sat within 2 % of wavelength/4. 1.5 mm of range on the quantity under test |
...
- H2's *"TE011 must barely move"* caught the wavelength/4 hybridisation
...
### 🔑 A WAVE PORT NEEDS **PEC** ON THE CONDUCTORS THAT BOUND IT (2026-09-03)

Palace's wave port solves a 2D eigenproblem on the port face to find its modal
field. With this programme's normal boundaries — `Conductivity` (aluminium wall
3.5e7, copper loop 5.8e7) — that solve **never converges**:

    Calculating boundary modes at wave ports for omega/2pi = 2.438e+00 GHz
    ... 39 min, no progress, no error, CPU 99.8%

Same mesh, same port, `Conductivity` swapped for `PEC` on the bounding
attributes: converges in seconds and writes S11.

🔴 IT LOOKS LIKE A HANG AND IS NOT ONE — no error is ever raised. Three wrong
theories were chased first (a coincident inserted face, sectors > 1, port-face
resolution) before the boundary condition was suspected.

✅ **This is usable, not merely a workaround.** With PEC walls there is no wall
loss, so Q_L collapses to Q_ext (up to dielectric loss in the torch) — the wave
port measures the COUPLING directly, with no need to separate Q0.
⚠️ So a wave-port run is NOT comparable to a lossy-wall run's Q_L. Use it for
Q_ext; take Q0 from an eigen solve with the port shorted.

### 🔴 THE PRIOR-ART-ON-FAILURE RULE EXISTS AND WAS VIOLATED TWICE (2026-09-03)

This file already says: *"When a solve fails, the FIRST question is 'has this
failed before?', not 'what do the logs say?'"* It was violated twice in one
session, both times costing hours:

  - **the viewport's `ns = 1` constraint.** `geometry.py` records that a radial
    stub through the barrel wall "has therefore only ever been simulated at
    ns = 1". Every coax attempt used `--sectors 5`. The note was in the file
    being edited.
  - **R4's eps-near-zero conditioning.** `NEXT.md` R4 records `h3_qext` timing
    out **with a vacuum torch** at eps = -1.46 — the same eps, torch and PCG
    stagnation later hit with the plasma — and names the remedy (a
    PRECONDITIONER). It was written up as EXTENDING the prior art. It reproduces
    it.

🔑 **THE RULE WAS NOT MISSING. A TRIGGER WAS.** A principle at line ~1528 of a
1500-line file does not fire. ➡️ `ops/priorart.sh <symptom>` searches, in the
order the answer is usually found: NEXT.md R-items, KNOWN.md PRIOR ART,
CONVENTIONS, INSTRUMENT, HYPOTHESES, then every rig and geometry.py.
**Run it before forming a hypothesis about a failure, not after.**

⚠️ **AND SEARCH BOTH ALPHABETS.** The record mixes Greek and ASCII — R4 writes
"ε-near-zero" while the rigs write "eps-near-zero"; likewise λ/lambda,
ω/omega, χ/chi. A single-form search misses the other, which is precisely how R4
stayed hidden. `priorart.sh` transliterates each term both ways.

## 7br — a label keyed on a LITERAL will eventually contradict the value it labels

`geometry.py` bound the torch permittivity from `baselines.json` (**9.39**,
eps_PERP_c) and then printed a label chosen by `abs(eps - 11.6) < 0.3` — a
literal of the *other* anisotropy axis. Result: every correctly-built mesh
announced `torch: custom`, and a mesh built on the WRONG axis would have
announced `SAPPHIRE`. The measurement was right and the label was **inverted**.

This is §"measurement vs evaluation" in one line of code: the value came from
the canonical source, the verdict about it came from a copy, and the copy went
stale when the canonical value was corrected. **Nothing errored.** Anyone
reading the mesh header would have concluded the wrong material was meshed.

➡️ **Label from the same source you bind from.** If a check says "is this X",
it must compare against X's canonical value, not a remembered number.

⚠️ **And expect the fail-closed guard to fire during the repair** — it did here:
`torch.quartz.permittivity` is TENTATIVE, so binding it plainly would have
hard-exited at mesh time. That is the guard working. Pass `allow_tentative=True`
*and say why at the call site* (here: it is a label, never meshed from).

## 7bs — decontaminate a count before letting it justify a rewrite

The naming audit proposed normalising λ to ASCII on the evidence
`lambda(214) λ(49)`. **168 of those 214 are Python's `lambda` keyword.** True
wavelength usage was 51 ASCII vs 54 Greek — the opposite conclusion. Executing
the sweep would have merged the wavelength into 168 lambda expressions and made
it permanently unsearchable, *in a change whose stated purpose was
searchability*.

Same trap, same corpus: `eta` hides inside `beta`, `meta`, `theta`; `chi` hides
inside `machine`; `nu` hides inside `number`.

➡️ **A bare frequency count over a mixed code/prose corpus is evidence about
STRINGS, not about usage.** Before a count authorises an edit, subtract the
homographs and show the residue. One `grep -c` is a hypothesis, not a finding.

## 7bt — prior art governs REPAIRS too, not only derivations

§7 tells you to search before deriving a setting. It applies just as hard to
fixing something. The recorded plan for the two-collision-frequency defect was
"add `plasma.collision_frequency.rad_per_s` to `baselines.json`". Searching
first found `physics.plasma_state`, written as the fix for that exact defect —
its header says the code "used to set NE and NU_M independently, and nothing
errored when they disagreed". n_e, nu_m and n_gas are **one state from one
temperature**; a standalone row would have minted a THIRD source of truth while
claiming to remove the second.

Likewise, the plan to sweep Greek/ASCII prose duplicated `ops/priorart.sh`,
adopted for the same reason — destructively, and across 758 lines.

➡️ **Before repairing, search for the repair.** Both fixes already existed and
had been documented; neither had been adopted by the code that needed them. The
failure was never a missing solution, it was a missing search.

🔑 **A tooling fix usually beats a corpus rewrite.** The prose sweep was
rejected; two silent bugs in `priorart.sh` were fixed instead (`η`, `ν`, `σ` and
the `₀` subscripts were missing from its table, and no entry had word
boundaries). Nine symbol pairs now return identical result sets.

## 7bu — when a symbol has THREE meanings, spell the word out

> User, 2026-09-03: *"just use wavelength instead of lambda/λ"*

§7bs stopped a sweep that would have normalised `λ` to `lambda`. The reason it
was wrong is worth stating separately, because it decides what to do next time:
**`λ` here carried three distinct meanings** — the wavelength (96 prose uses),
Python's `lambda` keyword (169 live expressions), and the **eigenvalue** of a
shift-invert transform (`1/(λ − σ)`, `INSTRUMENT.md` and `h3_ladder`). Picking
either symbol leaves two meanings sharing one string.

➡️ **A two-way collision can be disambiguated by convention** (*"never write a
bare β"*, *"never write a bare ν"*). **A three-way collision is cheaper to
retire**: write the word. `wavelength` now means the wavelength and nothing
else; the eigenvalue sites say *eigenvalue*; the keyword was never touched.

🔴 **AND RETIRE IT IN THE SEARCH TOOL AT THE SAME COMMIT.** Git history, the
literature and every earlier document still carry `λ`. Renaming without adding
`wavelength`/`lambda`/`λ` as one group to `ops/priorart.sh` would have made the
rename *itself* the next silent search failure — the exact fault the tool exists
to prevent (§7bt).

⚠️ **A token filter is not a guarantee — the post-check is.** The Python pass
edited only `COMMENT`/`STRING` tokens to protect the keyword. On Python 3.12,
f-string literal text is `FSTRING_MIDDLE`, **not** `STRING`, so 9 sites were
skipped in silence — `λ/4` inside `print(f"...")`. Counting what should have
been zero found them; nothing else would have. **Every mechanical sweep ends
with a count of what must now be absent, and that count is the deliverable, not
the sweep.**

⚠️ **Carry the subscripts' meaning across.** `λ₀` became *"(free-space
wavelength)"*, not `wavelength_0`: the subscript said free-space, and a bare
symbol swap would have dropped it while looking complete.

## 7bv — a launcher that cannot reach the host must FAIL, not narrate

`ops/go` classified the instance as `unreachable`, printed *"sync: skipped
(instance unreachable)"*, and **continued**. It then ran the lint, the ranks
check and the rsync — which failed with code 255 into a `| tail -2` pipeline
that swallowed the status — and the whole invocation **exited 0**.

🔴 **Exit 0 from a launcher means "your rig is running."** Here it meant
"nothing happened." The only evidence was an rsync error buried mid-log, and the
next step after a launch is to walk away and watch — so a silent no-op is
indistinguishable from a run in flight until the watch never produces anything.
This is §8b (*a rig is not done when it exits 0*) turned around: **a rig is not
STARTED just because the launcher exited 0.**

✅ Fixed 2026-09-03: the `unreachable` branch prints the host, says plainly that
nothing was synced and nothing will be launched, and **exits 3**. Verified
against a genuinely dead instance.

⚠️ **And check the exit code without a pipe.** `ops/go ... | sed` reports
`sed`'s status, which is how the first verification of this very fix read 0 on a
run that had correctly exited 3.

## 7bw — when reclamation is routine, resume is not a nicety, it is feasibility

2026-09-03: three instances in one session, reclaimed at **17.8 min**, and one
that vanished before its first solve. The volume's own log shows the spread —
35 min, 12.8 h, 27 h, 17.8 min — so lifetime is not something to plan around.

`h3_driven` had **no resume**. Its cold case alone takes 662 s, so on an
18-minute instance every relaunch re-solved case 1 and died before reaching the
deliverable. **The sweep was not slow, it was impossible.** `h3_loopq` had grown
resume after being killed twice; the pattern was sitting there unported.

➡️ **Any rig whose per-case cost is a meaningful fraction of an instance
lifetime needs case-level resume before it needs anything else.** Ported
`h3_loopq.resume_set` verbatim in spirit: keyed on slug+stamp (a config change
empties the resume set automatically), **complete means a real fit — errors are
RETRIED**, half-written records re-run.

🔴 **AND RESTORE THE CONTINUATION, NOT JUST THE RESULT.** `h3_driven` selects
each dip by continuation from the previous case. Skipping a completed case
without advancing `expect` would seed the next case from the COLD TE011 and walk
the continuation onto the wrong dip — a resume that silently corrupts the run it
was added to protect. The checkpoint already stored `expect_ghz`; resume reads it
back. **Test the restored STATE, not just the skip.**

✅ **And test resume against a REAL checkpoint before relying on it** — the one
from the reclaimed run, not a fabricated one. Verified: 1 case found,
`expect_ghz` carried, empty (not a crash) when disabled or absent.

## 7bx — a derived quantity must be reconstructible from the raw data beside it

2026-09-03, found by the user: `h3_driven` wrote `Q0 = 1/(1/Q_L - 1/Q_EXT_EST)`
where `Q_EXT_EST` is a **constant** — the plain 11x8 CAP loop's cold external Q —
on a barrel cavity with a series gap whose own Q_ext is 322. Every Q0 it emitted
was Q_L corrected by an assumption about **the quantity the run existed to
measure**. A Q_ext then derived as `Q0/beta` is circular.

🔴 **AND THE ERROR FLATTERED THE HYPOTHESIS.** The circular Q_ext came out at
264 against a cold 270 — a 2 % "agreement" that I reported as the assumption
being confirmed. **The wrong arithmetic produced exactly the answer I wanted**,
which is the condition under which a check is least likely to be run.

➡️ **RULE: a derived quantity must be reconstructible from the raw measurements
stored in the same artefact.** If `Q0` cannot be rebuilt from `Q_L` and `beta`
recorded beside it, the file is not a measurement — it is a measurement plus an
invisible assumption.

✅ **`verify_identities.py`** checks this over the corpus: `Q0 = Q_L(1+beta)`,
`Q_ext = Q0/beta`, `Q0_looped <= Q0_bare`. Exit 1 on failure, so it can gate a
workflow. First run: **90 files, 188 checks, 10 failures in 3 files, all with
the same implied constant.** Run it after ANY change to a derivation and before
quoting a Q out of an artefact.

🔑 **NONE OF IT NEEDED A SOLVER.** Not a cluster, not an instance that survives
18 minutes — one page of arithmetic nobody had written. **The cheapest check in
the programme was the missing one**, and the expensive infrastructure around it
was all working correctly.

⚠️ **Consistency is NECESSARY, NOT SUFFICIENT.** A run can satisfy every
identity and still have meshed the wrong cavity or named the wrong mode. This
check retires one failure class; it is not a warrant.

## 7by — a guard that sets a flag and never reads it is not a guard

2026-09-04, `h3-gap2load-02`. `check_eta_reference()` did its job: it detected
that `eta.reference` = 43,523 is for an **11x8 CAP loop** while the run meshes an
**11x8 barrel**, printed *"eta SUPPRESSED"*, and set `ETA_VALID = False`.

**Nothing ever read `ETA_VALID`.** The per-case code computed
`eta = 1 - Q0/qref` regardless and the summary line printed **`eta=0.9985`** — on
a SAPPHIRE BARREL cavity, against a VACUUM CAP-LOOP EIGEN reference.
**Cross-solver and cross-cavity at once**, which is the exact §7c trap the guard
was written to prevent. The warning scrolled past at line 1 of a 130-line log and
the number appeared 130 lines later looking like a measurement.

🔴 **AND THE NAME LIED TOO.** The value comes from
`out["q_ref_measured"] = Q0_COLD_EIGEN` — a CONSTANT — while the comment
immediately above its use reads *"SAME MESH, SAME SOLVER, SAME CAVITY"*. A field
called `q_ref_measured` that nothing in the run measured.

➡️ **RULE: a guard must GATE, not announce.** If a check can conclude "do not
quote this", the code path that produces the quantity must consult that
conclusion. A flag with no reader is a comment with extra steps.

✅ Fixed: the eta branch now tests `ETA_VALID` first and records `eta_error`
explaining the suppression, so the artefact carries the reason rather than a
number that reads as valid.

🔑 **THIS IS THE THIRD INSTANCE OF ONE PATTERN IN TWO DAYS** — `Q_EXT_MEASURED`,
`--torch-material`, and now `q_ref_measured`: **a constant from another cavity,
wearing a name that claims it was measured here.** The measurement layer keeps
being right and the layer that labels it keeps being wrong. When auditing, go
after the nouns: any identifier containing `measured`, `anchor`, `ref` or
`est` is a claim about provenance, and the claim is checkable.

## 7bz — the store cannot object to a coordinate the caller never named

> User, 2026-09-04: *"It seems there's another layer of variable name
> confounding. We have all these different values for different couplers,
> different cavity states, and different torch materials (and more, probably).
> It's really easy to lose track"*

**The configuration space is bigger than the names.** A Q is a point in at least
six coordinates — state (cold/loaded), density, cavity (torch), mount
(cap/barrel/azim), series gap, solver — and the NAME carries none of them.

🔑 **`values.get` already refused an AMBIGUOUS context and an UNKNOWN one.** The
gap it could not see was an **UNDER-SPECIFIED QUERY**. `h3_driven` asked
`cavity.Q_ext.cold` for (solver, mesh, ne) and got **9,231 — a CAP-loop value —
while meshing a barrel with a 2.25 mm series gap.** Nothing was ambiguous. The
question was. **A store cannot object to a coordinate the caller never named.**

✅ **FIX: a quantity DECLARES its mandatory coordinates.** `required_context` on
each entry; `values.get` refuses when the query omits one. `cavity.Q_ext.cold`,
`cavity.Q0.cold` and `cavity.Q0.loaded` now require **mount, gap2_mm, loop_mm**
(and ne_per_m3 when loaded), and the recorded rows were annotated with the
geometry they were measured on — the cap/no-gap fact previously lived only in
prose.

**Verified end to end:**

    barrel + gap2 2.25  ->  Q_ext = None, Q_EXT_VALID = False   (correctly refused)
    cap, no gap         ->  Q_ext = 9,231                        (still works)

🔑 **AND IT IMMEDIATELY FOUND A SECOND ONE.** Turning it on broke
`Q0_COLD_EIGEN`, an under-specified `cavity.Q0.cold` lookup nobody had noticed.
That is the point: **an enforced schema finds the other call sites for you.**

➡️ **DELETE THE HAND-WRITTEN GUARD ONCE THE STORE CAN ANSWER.** The first version
of this fix compared mount/gap2 in the rig — a SECOND copy of the store's schema,
free to drift from it. It is now one `try/except` around the lookup: the store
either has this geometry's value or refuses, and the refusal text is printed
verbatim. **A guard that restates the schema is the defect it is guarding
against.**

⚠️ **WHAT THIS DOES NOT FIX:** prose. A table headed "Q_ext" in a document still
carries no coordinates, which is why `Q_LEDGER.md` exists and why every Q quoted
in text needs state + cavity + loop, and density when loaded.

## 7ca — append-only is for DATA. A superseded CLAIM must be stamped in place

> User, 2026-09-04: *"This is another area where absolute adherence to
> 'append-only' did real damage. At the bare minimum, if a prior result is
> invalidated, it has to be modified to include an explanation as to why, and a
> forward pointer to the overriding result"*

**Correct, and it is the mechanism behind most of this week's confusion.**
Retractions were appended at the TOP of `NEXT.md` while the original entries sat
unchanged 90+ lines below, still headed **`## ✅✅ ... IS MEASURED`**. A reader
arriving at the original — by search, by scroll, by citation — sees a
double-tick claim with nothing to say it has been overturned. **The correction is
findable only by someone who already knows it exists**, which is precisely the
person who does not need it.

🔑 **THE DISTINCTION THAT MAKES APPEND-ONLY WORK:**

| what | rule |
|---|---|
| **measurements, result files, run configs** | **APPEND-ONLY, never edited.** A stamped config orphans its artefacts; an edited result is not a measurement. |
| **claims, verdicts, conclusions in prose** | **STAMP IN PLACE.** They are evaluation, and evaluation is re-runnable by design. |

➡️ **A SUPERSEDED CLAIM MUST CARRY, AT ITS OWN HEADING:**
1. that it is superseded — visible in the heading, not the body;
2. **WHY** — the specific defect, not "see below";
3. a **FORWARD POINTER** to what replaces it;
4. **what SURVIVES** — usually the raw measurement, even when the conclusion dies.

⚠️ **POINT 4 IS THE ONE THAT GETS SKIPPED AND MATTERS MOST.** `h3-gap2load-01`'s
Q_ext values are void and its `Q_L` and `coupling.beta` are perfectly good. A
retraction that does not separate them either destroys usable data or leaves
void numbers in circulation.

✅ Applied 2026-09-04 to *"cavity.Q_ext.loaded IS MEASURED"* and to *THE ORDER*,
whose premise ("Q_ext is cold, geometric") is now in doubt. The original text is
kept below each stamp, unedited, under *"The original entry follows, for
provenance"* — **stamping the claim, preserving the record.**

🔴 **AND THE HEADING IS THE INTERFACE.** `## ✅✅ X IS MEASURED` is what search
returns and what a future reader quotes. Leaving a ✅✅ on a dead claim is not
...
`ops/watch.sh`, so launching and watching became one operation. The launch was
then hosted in a backgrounded shell — and **that shell is capped by its HOST's
timeout, not by the solve's length.** It was killed with `rc=124` about four
minutes into a solve expected to run ~5 hours.

⚠️ **Nothing failed.** The rig is `nohup`'d on the instance and kept solving at
64 ranks throughout. That is what makes this dangerous: the only symptom was a
completion notification, and a completion notification is what a FINISHED run
produces too. The watch reported "done" when it meant **"I stopped looking."**

🔑 **Three properties, and the third is the one that was missing.** A watch must
be notified on solve exit, notified on instance exit, and **outlive both**. The
first two were built on 2026-09-04; the third was assumed.

🔴 **AND THE SAME DEFECT WAS ALREADY INSIDE THE WATCHER.** `watch.sh` returned
`rc=10` on a case turnover and printed *"Re-arm to watch the next one"* — advice,
issued at the exact moment it stopped watching. Identical in shape to
`remote.sh` printing the watch command as advice (§7cb), and identical in
outcome: **a step that must be remembered will eventually not be.** It now loops
and re-arms itself; the `rc=10` branch is unreachable and says so.

✅ `EVENTS=1` filters the stream to lines worth a notification so the watch can
live under a long-running event host. ⚠️ Its pattern covers **failure as well as
progress** — a filter matching only success is silent through a crash, and
silence is indistinguishable from "still solving". The internal pipe is legal
only because of `stdbuf -oL` / `--line-buffered`; the no-piping rule is about
BUFFERING callers, which is what hid a live watch twice.

## 7cd — a timeout is a runtime property; putting it in the fingerprint orphans the results

> User, 2026-09-04: *"Really? Why is the 12000s cap not resettable"* — and then,
> exactly: *"That's just bad design. There's no reason why the timeout should be
> tightly coupled when cancelling is just a simple kill."*

**The coupling was worse than it first looked.** Resume is keyed on
`sha256(baseline-<slug>.json)`, and `case_timeout_s` lives in that file. So
raising the cap **changed the input fingerprint**, and every case already solved
under the old stamp became unreachable.

Concretely, on 2026-09-05: `h3-betaconv2-0p8` had its **COLD case complete** and
its LOADED case killed by reclamation. The loaded case needs ~17,000 s at the
measured 429 s/sample, against a 12,000 s cap. Editing the cap to finish the run
would have discarded the finished cold case — **paying a re-solve for a number
that cannot affect a result.**

🔑 **A timeout cannot change a number. It can only decide whether a number is
obtained.** It is therefore not an input to the physics and does not belong in
the input fingerprint. `$AMIP_CASE_TIMEOUT_S` overrides it with the config file
untouched, so the stamp still matches and solved cases are kept. ⚠️ The
**effective** cap goes into every record — otherwise a case truncated at the cap
is indistinguishable from one that converged, which is the §7by failure again.

🔴 **AND THE OVERRIDE NEARLY DID NOTHING.** `ops/remote.sh` hardcoded
`PALACE_RANKS` and `RUN` into its `ssh` line, so **every other exported variable
was silently dropped** — the rig would have run its 12,000 s default and said
nothing. Env does not cross `ssh` by itself. It now forwards an explicit list and
**prints what it forwarded**, so the launch banner shows the run's actual
settings rather than the intended ones.

## 7ce — an armed, correct, SILENT watch: the rig log is not where progress lives

**2026-09-05.** The watch from §7cc was re-hosted so it could outlive the solve,
and then it emitted **nothing for an hour**. It was working. `watchrig.sh`
mirrors the RIG log, and the rig prints per **case** — one block on entry, one on
exit — so a 5-hour driven case is a 5-hour silence by construction.

🔴 **The user supplied the missing signal:** *"1h again, 80GB steady."* That is
the *polls for progress* criterion failing in the field, and it failed in the
worst possible way — with a watch that looked healthy, because it WAS healthy.

✅ Palace's own log is live and carries the one number that matters: the adaptive
PROM's **greedy iteration, sample count and error indicator**, directly
comparable against `AdaptiveTol`. `ops/solverprogress.sh` polls it, and
`ops/watch.sh` STARTS IT ITSELF in `EVENTS` mode — documenting it would have
been another step to remember (§7cb, §7cc).

⚠️ **AND THE FIRST VERSION FLOODED.** It emitted "only on change", comparing the
whole line — which carries memory and elapsed time, so *every poll* was a change.
**Two identical greedy iterations 22 s apart.** Caught by SMOKE-TESTING the
poller, not by reading it: `bash -n` and `preflight` both passed it clean. The
dedup key is now the greedy line alone, with a rare heartbeat so a STALLED
iteration still breaks silence rather than reproducing §7cc.

🔑 **The general rule: a watcher's output is a measurement, and an unproven
filter is an unproven instrument.** "It parses" and "it emits the right thing"
are different claims, and only one of them was checked.

## 7cf — `pkill -f` matches the shell that runs it

**2026-09-05, and the second time.** Clearing stale watchers with
`pkill -f "watchrig.sh /opt"` killed the invoking shell: the pattern appears
verbatim in that shell's own command line, so it matched itself. `rc=144`.

⚠️ Identical to the earlier `pkill -f "h3_driven.py"` which matched its own ssh
command. Writing it down did not stop it, because the two look nothing alike at
the call site — only the mechanism is shared.

✅ Either exclude `$$` explicitly, or build the pattern so the shell's own
arguments never contain it verbatim (`PAT="watchrig"'\.sh'`). `ps -C` remains
right for a known binary, but ⚠️ Linux truncates `comm` to 15 characters, which
is how `palace-x86_64.b` was missed once.

## 7cg — a dependency's config key quoted from memory, and the schema ships with the build

**2026-09-05.** `h3-betaconv3-0p8` tested the hypothesis that bounding the GMRES
Krylov basis would cut memory enough to extend the mesh convergence series below
sf 0.8. Both cases died **in 2 seconds**: `MFEM abort: Configuration file
validation failed` (`iodata.cpp:212`). Not memory, not conditioning — **Palace
refused the config.**

`solveconf.py` emitted `Solver.Linear.Restart`. Read from the installed source
rather than from recollection:

| where | key | meaning |
|---|---|---|
| `configfile.cpp:1370-71` | `Linear.MaxIts`, `Linear.MaxSize` | `MaxSize` **is** the GMRES(m) restart dimension |
| `configfile.cpp:1107` | `Driven.Restart` | a different setting entirely |
| `configfile.cpp:1120` | — | `'"Restart" is incompatible with adaptive frequency sweep!'` |

🔴 **So the key was invented, and the one it resembled could never have worked
here anyway** — every rig in this programme uses the adaptive PROM, which Palace
explicitly refuses to combine with `Driven.Restart`. The *mechanism* reasoned
about in the comment block (one basis vector per iteration, memory scaling with
iteration count) was sound; only the knob's name was fabricated.

🔑 **This is the programme's own rule applied one level out.** "Search prior art
before deriving" and "a value must be bound, not copied" were both about THIS
repo's values. A dependency's configuration schema is the same class of fact —
and `config-schema.json` ships inside the build, so checking cost one grep.

⚠️ **AND IT EXITED 0.** The rig records a per-case error and carries on, so a
run in which *every case failed* still ended `EXIT=0` and the watch announced
"run finished". A launch that fails validation is indistinguishable, at the exit
code, from one that succeeded.

## 7ch — the convergence axis is not uniformly typed

**2026-09-05.** Assembling the four-point series, `size_factor` came back as
`'1.2'`, `'1'`, `'0.8'` — **strings** — in three of four artefacts, so sorting
the series by its own independent variable raised `TypeError`.

⚠️ Harmless here only because it crashed loudly. Had it silently sorted
lexicographically, `'1'` < `'1.2'` < `'0.8'` would have put the series in the
wrong ORDER and the step sizes — the entire result — would have been computed
between the wrong neighbours.

🔑 `OPTIMIZER.md` § THE SCHEMA CHANGE already says `size_factor` is a COORDINATE
of every observation. A coordinate that is a string in some records and a float
in others cannot be grouped, sorted or compared without a cast at every use.

## 7ci — the identity checker had been dead since the audit that made it necessary

**2026-09-05.** `verify_identities.py` — the page of arithmetic that found the
ten `Q_ext = 9,231` failures and is meant to GATE a workflow — **crashed at
import**:

    values.Unknown: 'cavity.Q0.cold' requires context
    ['mount','gap2_mm','loop_mm','solver','mesh']; missing ['mount','gap2_mm','loop_mm']

Its module-level `BARE_Q0_MAX` lookup was written before `cavity.Q0.cold`
declared `required_context`. The underspecification audit — the work that made
the store refuse ambiguous names — **broke the checker that the same audit
existed to support**, and nothing reported it, because a gate that cannot start
does not fail loudly. It fails by not being there.

🔴 **Found only by running it against a new artefact**, not by any check. It sat
dead across every run of 2026-09-04 and 2026-09-05.

✅ Repaired by naming the coordinates. Result: **98 files, 196 checks, 16
failures** (was 10 — the extra six are newer artefacts, same `9,231` signature).
The three new mesh-convergence points pass outright.

🔑 **AND IT IMMEDIATELY EARNED ITS KEEP.** It flagged `h3-gap2load-02`, the
COARSEST point of the convergence series published minutes earlier — stored
`Q0` = 60.99 against `Q_L(1+beta)` = 75.58. The series used `wide_fit`, so the
number quoted was the correct one; but that was luck of habit, not verification,
until the checker said so. ⚠️ Anyone reading that artefact's `Q0` field directly
gets a value 19 % low.

⚠️ **A checker is not a check until something runs it.** `preflight` runs on
every launch; `verify_identities` runs when someone remembers. That is the whole
difference, and it is §7.0 again.

## 7cj — `set -e` plus a timeout deleted the guarantee that launch and watch are one operation

**2026-09-05.** §7cb replaced "print the watch command as advice" with
`exec`-ing into `ops/watch.sh`, so that no window exists in which a run is live
and unwatched. Two Monitor-hosted launches then stopped at exactly:

    launched h3_driven.py at 64 ranks
    [exited with code 124]

**and watched nothing.** `ops/remote.sh` runs under `set -euo pipefail`. The
launch ssh sometimes does not return — it holds the channel even though the job
is `nohup`-detached and the remote `echo` has already printed — so `timeout 60`
kills it, 124 meets `set -e`, and the script dies **after launching and before
the exec.**

🔑 **The guarantee was resting on an ssh that sometimes does not come back**, and
a guarantee with a hole in it is worse than no guarantee: it stops the next
person looking for the hole. I had already written §7cb claiming the window was
closed.

⚠️ **The isolated test said the opposite.** Reproducing the launch shape with
`nohup bash -c 'sleep 45'` returned in 3 s, rc=0 — three variants, all clean. The
hang needs the real rig, so the experiment that "cleared" the ssh was measuring
something else. A negative result from a simplified reproduction is not a
negative result.

✅ **The launch ssh's exit code is not evidence either way.** It now captures the
status without dying, then ASKS THE INSTANCE whether the rig is running, and
exits 5 if it is not. Verify with the consumer, not the messenger.

⚠️ `ps -C python3 -o args= | grep -c -- '-u <rig>'`, deliberately **not**
`pgrep -f` — pgrep's own argv contains the rig name and it matches itself
(§7cf, which had already happened twice).

## 7ck — the report layer assumed the fit had succeeded, and killed the run after every solve

**2026-09-05, `h3-azimload-02`.** Both cases solved. The rig then died in
`_report`:

    File "h3_driven.py", line 1195, in resolve_branch
      qL, b_u = w["Q_L"], w["beta"]
    KeyError: 'Q_L'
    EXIT=1

The LOADED azimuthal dip has **no measurable 3 dB width** — walking down from f0
ran into the 2.30 GHz band edge, and walking up the curve TURNED at 2.4546 GHz
before reaching −3 dB. So `wide_fit` correctly carried `beta` and `s11_db` and
**no `Q_L`**. `resolve_branch` indexed it unconditionally.

🔑 **The measurement layer was right and the evaluation layer was wrong** — the
exact split this programme keeps paying for. The fit REFUSED, honestly and with
a reason; the thing that consumed it assumed success. Every solve completed and
the run still produced no report and `EXIT=1`.

⚠️ **Refuse, do not substitute.** `beta` comes from the DIP DEPTH and is still
sound (0.2053); without `Q_L` there is no `cavity.Q0.loaded` and no
`cavity.Q_ext.loaded`. Manufacturing a width from the band edge would have
invented both, and they would have looked like measurements.

✅ Now returns `{resolved: False, Q0: None, Q_ext: None, why: ...}`. Verified by
running the real artefact through the repaired function — the case that killed
the run returns a refusal.

⚠️ **And a `None` reaching a formatter is the §7-class crash on the other side**
— `_q()` exists for exactly this, which is why the fix returns `None` rather
than `nan`.

## 7cl — "converged" carried two unrelated meanings, in the same documents, in the same session

**2026-09-06.** I wrote *"`coupling.beta.loaded` genuinely does not converge"* and
the user read it the other way: **user:** *"this means their Q_ext doesn't agree at
different size factors, not that the solves failed."* Correct — and the sentence
does not say so.

**Two claims, and they have opposite failure modes:**

| | is a property of | its failure is |
|---|---|---|
| **solver-converged** | ONE run — PROM under `AdaptiveTol`, GMRES, NLEPS | an **ERROR**: rc≠0, no result, *"Linear solver did not converge"* |
| **mesh-converged** | a SERIES — does the value settle as `size_factor` falls | a **RESULT**: every solve succeeded and the numbers still disagree |

🔴 **BOTH WERE LIVE IN THE SAME SESSION AND WERE ADJACENT IN THE SAME LEDGER.**
`h3-betaconv3-0p8` (MaxSize=30) **solver**-failed — 21 greedy iterations that
never reached `AdaptiveTol`. The four-point series **mesh**-failed — every run
reached `AdaptiveTol` cleanly and the answers still moved 8.56 %. Writing "did not
converge" for both invites exactly the wrong inference: that the series is
untrustworthy because something broke, when in fact nothing broke and that is
precisely what makes the result meaningful.

⚠️ **84 uses of "converg*" across `Q_LEDGER`/`NEXT`/`KNOWN`/`CONVENTIONS`, and
`GLOSSARY.md` had no entry** — while itself using BOTH senses two lines apart
("not mesh-converged" at one entry, "an evaluation that did not converge" at
another). The document whose stated purpose is *"say exactly this, mean exactly
this"* was carrying the ambiguity it exists to remove.

✅ Split in `GLOSSARY.md` as **solver-converged** vs **mesh-converged**. Same
disease as §7bu (a symbol with three meanings) applied to a WORD.

## 7cm — the wrong Bessel constant, and the one-line check that would have caught it

**2026-09-06.** `coldfield.py` estimated |E| inside the torch to decide whether a
COLD match is worth buying. It used `KC_A = 2.405` — the first zero of **J0**,
which is a **TM01** constant. TE011's transverse profile is `J1(chi'01 r/a)` with
**chi'01 = 3.8317**, the first zero of **J1 (= J0')**.

🔴 **EVERY |E| IT PRINTED WAS WRONG**, and the numbers were quoted to the user and
written into `NEXT.md` as the basis of a design decision. Corrected, the field at
r = 8.5 mm under a perfect cold match is **2.20 kV/cm, not 1.08** — which moved
the conclusion from *"falls short by 2–20×"* to *"reaches the bottom of the target
band at the outer radius."* The decision survived; the stated margin did not.

✅ **THE CHECK IS ONE LINE AND IS NOW A `self_test()`:** the constant must
reproduce the cavity's DESIGN FREQUENCY.

    chi'01 = 3.8317 -> f_TE011 = 2.4500 GHz   ✅ the cavity is built for this
    2.405           -> f_TE011 = 1.8404 GHz   🔴 25% off, and unmissable

🔑 **A DERIVED CONSTANT MUST BE ANCHORED TO A KNOWN OUTPUT.** `physics.f_mnp` and
`physics.bessel_zeros` already exist and would have supplied the right value; the
estimate hardcoded its own instead — the same "copied, not bound" defect the
programme has recorded repeatedly, applied to a mathematical constant.

⚠️ **FOUND BY READING PRIOR ART, NOT BY CHECKING.** `probecheck.py` states the
profile correctly and gives the peak at `r = 0.4805a`; I read it while looking for
something else. Nothing in the estimate would ever have flagged itself.

## 7cn — a claim the user had already corrected, restated an hour later

**2026-09-06.** I wrote *"there is no magnitude tuner"* into `NEXT.md` as an
argument FOR optimising the coupler on the loaded state. The user corrected it:
*"it says its unsolved, not that it doesn't exist."* I retracted it in place,
recorded the retraction, and then — in the same session, roughly an hour later —
**wrote the same claim again**: *"The coupler is FIXED geometry (no magnitude
tuner), so it matches at ONE sigma."* The user corrected it a second time.

🔴 **THE RETRACTION WAS IN THE FILE AND DID NOT PREVENT THE REPEAT.** Both
sentences are in `NEXT.md`, ~280 lines apart: one saying the claim was an
overstatement and is withdrawn, the other making it again as a premise. Writing
the correction down is not the same as consulting it.

⚠️ **AND IT PROPAGATED INTO REASONING.** The false premise produced a conclusion —
*"so the map must pick ONE match point"* — that is materially wrong. With a tuner
the deliverable is a **RANGE** of `cavity.Q_ext` across the plasma trajectory,
which is a different (and more useful) analysis of the same run.

🔑 **DISTINCT FROM §7bt.** That rule is *search prior art before deriving OR
repairing* — about the RECORD. This is about the LIVE CONVERSATION: a correction
received minutes ago is the most authoritative prior art there is, and it is the
easiest to skip because it feels like something already known.

✅ The refinement worth keeping: **unsolved ≠ absent ≠ impossible**, and the
distinction changes what a rig is for. `../control-loop/` item 2 rejected four
PIN-diode candidates for a real thermal reason; that bounds the SOLUTION SPACE,
not the requirement.

## 7co — the None-format rule was ~80% dead since it was written, by regex precedence

**2026-09-06.** `h3-q0map-01` died at `{ne:.1e}` with
`TypeError: unsupported format string passed to NoneType.__format__` — **after a
1,699 s solve**, which is exactly the cost `r_none_format` exists to prevent. That
rule was created on 2026-09-04 because *"this exact shape killed three runs, each
after a solve."* It did not fire.

**Two faults, and the second is much worse than the first.**

🔴 **1 — THE RULE IS NAME-BASED AND THE NAME WAS NEW.** `_REFUSABLE` lists
literal names; I made `ne` refusable (None on the prescribed-(eps, sigma) path)
without adding it. ⚠️ **Making a value refusable is TWO edits**, and the second is
easy to skip because the code works until the None actually arrives.

🔴 **2 — AND THE GUARD CLAUSE HAD A PRECEDENCE BUG.** The escape test read:

    re.search(r"if\s+" + _REFUSABLE.pattern, line)

Alternation binds looser than concatenation, so that is

    (if\s+Q0_COLD_EIGEN) | Q_EXT_MEASURED | Q_EXT_EST | q0c | _q0ref

— every name AFTER the first matches **bare, anywhere on the line**. So any line
containing `Q_EXT_MEASURED` was treated as ALREADY GUARDED and skipped.
**The rule could only ever fire for `Q0_COLD_EIGEN` — one name of five.** It was
~80% dead from the day it was written, and nothing revealed that, because a rule
that never fires looks exactly like a codebase with no defects.
✅ Fixed with a non-capturing group. Verified it now fires for all five names AND
stays silent on each documented guard shape.

🔑 **A RULE'S SELF-TEST PROVED IT "DISCRIMINATES" THROUGHOUT.** `--self-test`
passed the whole time: its fixture used `Q0_COLD_EIGEN`, the one name that
worked. **A fixture that exercises one branch of an alternation certifies the
alternation.**

⚠️ **AND THE FIX FLAGGED 8 FALSE POSITIVES ACROSS 5 RIGS**, because `ne` is
refusable only in `h3_driven`; elsewhere it is always a real grid value. A noisy
rule is one people learn to ignore, so `_REFUSABLE_ONLY_IN` scopes a name to the
files where it can actually be None. **Absent from that map = refusable
everywhere**, which is the safe default.

⚠️ It then found a THIRD unguarded site in `h3_driven` that I had missed by hand.

## 7cp — the None-format rule could not see the house style

**2026-09-07, the same rule as §7co, a different blind spot.** `h3-q0map-01`
died in `_report` on `{p['ne']:>9.1e}` — **the FOURTH such site**, after 1,699 s
and eight completed solves. §7co had just fixed the rule's precedence bug and
added `ne` to its name list. It still missed this.

🔴 **THE SPEC PATTERN WAS `[,.0-9]*[fdge]` — NO FILL, NO ALIGNMENT.**

    {ne:.1e}          matched
    {ne:,.0f}         matched
    {ne:>9.1e}        MISSED   <- and `>` alignment is the house style for
    {ne:>8.2f}        MISSED      every table in this repo

So the rule was blind to the most common shape it exists to catch. ✅ Now accepts
any spec ENDING in a numeric presentation type; verified it still rejects
`{name:>9s}` and `{x!r}`.

🔑 **THREE ROUNDS, THREE DIFFERENT BLIND SPOTS, ONE RULE:** the name list (§7co),
the guard's operator precedence (§7co), and the spec grammar (here). Each fix
revealed the next, and every round the rule reported CLEAN in between.
⚠️ **A rule that has never fired has not been tested — it has only been run.**

✅ Widening it immediately found two more real sites AND a stale docstring:
`resume_set` still said *"as {ne: record}"* after the key was changed to the tag.
A lint aimed at format specs caught documentation drift, which is luck, not
design — but it is the kind of luck that only happens once the rule can see.

## 7cq — a branch bootstrap that assumed a low-loss coupler, and the constraint that caught it

**2026-09-07, `h3-azimmap-01`.** `Q0_if_overcoupled ≡ Q_ext_if_undercoupled`, so a
single dip cannot separate them. On the BARREL map the series broke the
degeneracy by consistency (§ Q_LEDGER), bootstrapped from the vacuum row with:
*"an empty grooved cavity has Q0 ~4e4, so the LARGER root is Q0 and the smaller
is Q_ext."*

🔴 **THAT REASONING SILENTLY ASSUMES A LOW-LOSS COUPLER, AND THE AZIMUTHAL LOOP IS
NOT ONE.** Applied to it, the vacuum row gave Q0 = 41,633 / Q_ext = 18,206. The
inverted assignment gives **Q0 = 18,206 / Q_ext = 41,633** — the loop is a large
conductor 2 mm off the wall in a high-H region and it EATS CAVITY Q.

✅ **WHAT DISCRIMINATED, AND IT IS MEASURED RATHER THAN ASSUMED.** The named
probes showed both couplers see the SAME mode: azimuthal profile
0.2357 / 0.5071 / 1 / 2.7438 against the barrel's 0.2396 / 0.5097 / 1 / 2.7431.
🔑 **Same mode shape ⇒ `sigma·Q_plasma` MUST be shared**, because K depends on how
much field sits in the plasma, not on the coupler. Only one assignment delivers
it:

    assignment A (larger = Q0):  K = 758.9   — 6x off the barrel's 125.8
    assignment B (smaller = Q0): K = 130.4   — within 4%  ✅

⚠️ **AND AN INDEPENDENT CHECK AGREED**: `h3-azimload-02`'s cold case gives
azimuthal Q0 = 14,074 at standoff 3.0, the same order as 18,206 at standoff 2.0.
Two routes, neither of them the assumption that failed.

🔑 **THE GENERAL RULE: PREFER A MEASURED INVARIANT TO A PHYSICAL PRIOR.** The
bootstrap encoded a belief about the cavity; the shared-K constraint encodes a
measurement of it. ⚠️ And K is only shared BECAUSE the probes established the mode
is — without them the constraint would itself have been an assumption.

⚠️ **CONSEQUENCE, NOT YET FULLY MEASURED:** the azimuthal arm carries a DOUBLE
penalty — ~4.7x weaker coupling AND a cavity Q0 more than halved by the loop's own
loss. A predicted 69.6% delivered, published from the wrong branch, is WITHDRAWN
pending the remaining rows.

## 7cr — a refinement field defined in the wrong coordinate, and an error indicator nobody read

**2026-09-06.** The azimuthal coupling loop produced a 44 % broadband absorption
in a **vacuum** cavity — impossible — and it survived four runs and several weeks
as three separately-diagnosed anomalies (`ret:azimuthal-driven-pedestal`).

The cause is one line. `geometry.py` refines the arc conductor with overlapping
Balls placed on the conductor **centreline**, radius `2.0 * loop_rw` = 2.0 mm.
But the balls' job is to resolve the conductor-to-**wall** field, and the wall is
`_hh` = standoff + t/2 from the centreline — 3.0 mm at standoff 2, 4.0 mm at
standoff 3. So the refinement stopped **1.0 mm short**, then **2.0 mm short**, in
exactly the gap where an azimuthal loop's field is strongest.

🔑 **THE RADIUS WAS WRITTEN IN THE CONDUCTOR'S COORDINATE AND THE REQUIREMENT
LIVES IN THE WALL'S.** `2.0 * _rc` is a perfectly sensible number for "cover the
conductor" and a meaningless one for "span the gap". Both readings are natural,
the code names neither, and the barrel loop — which has no wall gap — never
exposed the difference.

### The part that should have been caught in a day, not a month

**`error-indicators.csv` was written by every single solve, and nothing ever read
it.** At matched element counts it separates the two arms outright:

| | tets | baseline \|S11\| | err max |
|---|---|---|---|
| barrel | 346,118 | −0.010 dB | 5.85e-3 |
| azimuthal | 349,530 | −2.456 dB | **1.29e-1** |

Across all 40 runs with the file on disk the split is total: baseline ≈ 0 dB ⟺
`err max` ≤ 9.3e-2; a pedestal ⟺ `err max` ≥ 1.29e-1. And the two facts that
settle *mechanism* were also already on disk — the pedestal falls under
refinement on identical geometry (124,622 tets → −4.108 dB, 349,530 → −2.456),
and it tracks the unreached gap at fixed resolution (1.0 mm → 43 % absorbed,
2.0 mm → 58 %).

🔑 **THE SAME SHAPE AS VERIFY.md's FINDING, ONE LAYER DOWN.** There the
evaluation layer was re-runnable and nobody re-ran it. Here the solver was
*already reporting its own discretisation error*, in every artefact, and nobody
looked. **A diagnostic that is emitted but never read is not a diagnostic.**

### What hid it

- `h3_driven.build_mesh` runs the mesher with `capture_output=True` and discards
  stdout on success. geometry.py prints one line per refinement field it
  installs — so no artefact could say whether the arc field had fired, and
  establishing that it *had* took reading control flow (`p["loop_azim"]` is set
  at `:455` and read at `:1778`, both inside `build()`).
- `arc_chords` looked like a resolution knob and **was not a knob at all** — see
  §7cs. Chasing it cost a solve and, briefly, a wrong sentence in `Q_LEDGER.md`.
- MPI rank count appears in **no artefact**. Confirming both maps ran at 32 had
  to be done by inferring it from Palace's per-rank memory totals.

### The rules

🔑 **A REFINEMENT REGION MUST BE SIZED IN THE COORDINATE OF THE FEATURE IT
RESOLVES.** If the field exists to resolve a gap, its extent is a function of
that gap — not of the conductor that happens to be at its centre.

🔑 **READ THE SOLVER'S OWN ERROR ESTIMATE BEFORE BELIEVING A RESULT**, and read
it *comparatively* — against the same quantity on a geometry already trusted, at
a matched element count. An absolute error norm means little; 22× the barrel's at
1 % more elements means everything.

🔑 **A PHYSICALLY IMPOSSIBLE RESULT IS A MESH QUESTION BEFORE IT IS A PHYSICS
QUESTION.** Lossless cavity, 44 % absorbed, identical configuration to a clean
run: the only remaining variable was the mesh, and that was true on day one.


## 7cs — a parameter declared, plumbed, recorded, retried over, and consumed by nothing

**2026-09-06, immediately after §7cr.** Diagnosing the azimuthal pedestal I
hypothesised that the arc conductor's **facets** were the field singularity, on
the strength of `geometry.py`'s own `arc_chords` parameter and both rigs' loops
that "retry over the arc's chord count". I launched `h3-azimchord-01` at 11
chords against `h3-azimmap-01`'s 7.

**It came back bit-identical.** 349,530 tets, −2.456 dB, err max 1.29e-1 — every
digit.

🔴 **`arc_chords` HAD NO CONSUMER.** Four sites existed: a default in `P`, a read
from `AMIP_ARC_CHORDS`, a write into the mesh sidecar, and a write into the run
record. **No geometry code ever read it.** The arc is an OCC torus
(`occ.revolve`); the polyline-chord construction the flag is named for was built
in August and abandoned because Palace refused the mesh
(`MFEM abort: STable3D::operator()`). The comment naming that abandonment sits
40 lines from the construction, and I had read neither before designing a run
around the flag.

### Why it was so convincing

Every signal a parameter can give, it gave. It had a considered default. It had
an environment override. It was **recorded in the sidecar and in every run
record**, so `arc_chords: 7` appeared in artefacts as a fact about the geometry.
Two independent rigs retried over it with a comment explaining the physics —
*"no single count works for all h, and which one fails moves with every parameter
change (`h3_loopq.build_mesh` carries the measured table)"*. That table was
measuring nothing: every value produced the same mesh, so "7 worked" only ever
meant "the first attempt worked".

🔑 **A DEAD PARAMETER THAT IS RECORDED IS WORSE THAN ONE THAT IS IGNORED.**
Ignored, it does nothing. Recorded, it manufactures a coordinate — and the whole
programme's discipline is that observations carry their coordinates. Four weeks
of azimuthal artefacts assert a geometry that was never built.

### The rules

🔑 **BEFORE BUILDING A RUN AROUND A PARAMETER, FIND ITS CONSUMER.** Not its
declaration, not its docstring, not the code that stores it — the line that
*uses* it to change something. `grep` for the name and read every hit: here there
were four, and all four were plumbing.

🔑 **REFUSE A DEAD FLAG, NEVER IGNORE IT.** `geometry.py` already refuses
`--loop-azim` with an explanation of what replaced it and why. `AMIP_ARC_CHORDS`
now does the same. Ignoring is what let it record a false coordinate for a month.

🔑 **`P IS BUILT BY EXPLICIT ASSIGNMENT` CUTS BOTH WAYS.** The comment at
`geometry.py:2472` warns that a flag not copied into `P` silently never runs.
This is the mirror image — copied into `P`, and *still* never run, because
copying is not consuming. Both failures look like a working feature.

## 7ct — two mechanisms proposed and killed for a sink that was measurable all along

**2026-09-06/07.** The azimuthal arm absorbed 44 % of incident power in a vacuum
cavity. Over one session I proposed two mechanisms, built a run for each, and
killed both:

| hypothesis | run | outcome |
|---|---|---|
| arc conductor **faceting** | `h3-azimchord-01` | the flag had no consumer; there are no facets (§7cs) |
| unrefined conductor-to-**wall gap** | `h3-azimwall-01` | field widened 2.0 → 3.5 mm, confirmed in the record; baseline moved 0.011 dB |

Then I used `Boundaries.Postprocessing.SurfaceFlux` `Type: "Power"` — three lines
of config — and the answer arrived in one solve: **nothing is absorbing.** The
model dissipates 0.4 % of the claimed absorption off resonance, 23 % on it, while
the barrel control closes to 97 %.

🔑 **I WAS PROPOSING MECHANISMS FOR A QUANTITY NOBODY HAD MEASURED.** Both
hypotheses presupposed that 44 % of the power was going *somewhere* and argued
about where. Neither asked whether it was going anywhere at all — and that was
one config block away, in a solver feature this programme had never used.

### The reasoning error that cost the most

I ruled out the port face **because the `LumpedPort` configuration was identical
to the barrel's** — same `R`, same `Direction`, same attribute. I then wrote
*"DO NOT RE-TEST: the port face"* into `Q_LEDGER.md` **and** into
`retractions.json`.

🔴 The defect is in the **mesh face the port is attached to**. The azimuthal gap
faces are planes at ±psi, so the port face spanning them is an annular *sector*,
and Palace's lumped port requires a flat element. **No amount of configuration
diffing can see that**, because the configuration only names the attribute — the
*shape* of the attribute lives in the mesh. And `geometry.py` had documented it
as "the open problem" since the loop was built, thirty lines above an arc
construction I read that same morning.

**A confident exclusion written into the record is worse than no analysis**: it
does not merely fail to find the cause, it instructs the next reader not to look
there.

### The rules

🔑 **BEFORE THEORISING ABOUT WHERE SOMETHING GOES, CHECK WHETHER IT GOES
ANYWHERE.** A conservation check is cheaper than any mechanism, and it discards
whole families of hypothesis at once. Power in minus power out, against the
sinks, on a geometry already trusted.

🔑 **"THE CONFIGURATIONS ARE IDENTICAL" DOES NOT MEAN "THE MODELS ARE
IDENTICAL".** A config names attributes; the mesh gives them shape. Two runs can
agree in every solver field and still pose different problems.

🔑 **CALIBRATE A NEW MEASUREMENT ON A GEOMETRY THAT ALREADY WORKS, IN THE SAME
RUN SET.** The barrel control is what turned "the azimuthal surfaces sum to
0.0016 W" from a number into a verdict — and, unplanned, it settled the drive
normalisation (`P_inc` = 1 W) that had blocked absolute field quotes for weeks.
